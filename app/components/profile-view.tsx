...
//         {posts.map((post) => (
//           <div key={post.id} className="relative rounded-xl overflow-hidden shadow-lg transition duration-300 ease-in-out transform hover:scale-105">
//             <Image
//               src={post.image}
//               alt={post.title}
//               width={300}
...
//       {/* Add Post Button */}
//       <button className="fixed bottom-6 right-6 p-4 bg-blue-600 rounded-full text-white shadow-lg transition duration-300 ease-in-out transform hover:scale-110 hover:bg-blue-700">
...
export function ProfileView({ onAddPost }: ProfileViewProps) {
  const dispatch = useDispatch()
  const { name, bio, avatar, backgroundImage, posts } = useSelector((state: RootState) => state.profile)
